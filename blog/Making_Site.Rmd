...
I looked into [Wordpress](https://wordpress.com/) but I wasn't thrilled with the theme choices if I opted for free site hosting.  Then I realized that you can generate your site entirely using [RStudio](http://rmarkdown.rstudio.com/rmarkdown_websites.html), which seemed like a great option but had limited customization options. 

Then I stumbled upon [Blogdown](https://github.com/rstudio/blogdown), a new RStudio package developed by `knitr` and `bookdown` creator [Yihui Xie](https://github.com/yihui).  The package runs using a static-site generator called "[Hugo](https://gohugo.io/)" which, at that point, I had never heard of. And while the package was still under development and had no formal package documentation, I wanted to give it a try.  After several days of googling, experimenting, and learning, I am now the proud owner of a site I love.  This post details how I made it happen. 
...
Hugo is a static-site generator, meaning that instead of generating your website from scratch everytime someone visits your page, the pages are already made and ready to go when someone arrives [more info here](https://davidwalsh.name/introduction-static-site-generators). It's also open-source and free, which I support. 
...
![](Making_Site_images/frontmatter.png)

* **[Variables](https://gohugo.io/templates/variables/)**: These pieces of information often come from either front matter, the content itself, or where the file is located. (e.g., a file in the "blog" folder will automatically obtain the .Section variable "blog", while the .Title variable is defined by you in the front matter, and the .Summary variable is automatically generated by Hugo to include the first 70 words of your post).
...
Ok, all of that will be helpful as we continue! Now let's get started!
...
* I'm erring on the side of too much detail, so skip past things you know as needed.
...
1. If you don't already have one, create a [GitHub](https://github.com/) account. *Choose your username wisely as it will be in your new site URL*
...
newrepo <- image_read("Making_Site_images/newrepo.png")
newrepo
...
newrepo2 <- image_read("Making_Site_images/newrepo2.png")
newrepo2
```
...
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "50%"}
clonerepo2 <- image_read("Making_Site_images/clonerepo2.png")
clonerepo2
```
...
1. Launch RStudio and create a New Project. Select "Existing directory" and navigate to the folder above the working directory you just created from GitHub (in my case, it's a folder called Parker_Website). **The GitHub directory must be completely empty to create a new_site. If you accidentally put any files in there, move them out until after the new_site is created**
...
library(blogdown)
```
3. Intall Hugo. You can either do it manually ([see documentation](https://gohugo.io/overview/installing/)) or using `blogdown`.
```{r eval = FALSE}
install_hugo()
```
*Note: on my Mac, the first time I installed Hugo, I also needed to install [homebrew](http://brew.sh/)*

The blogdown way is definitely simpler than installing Hugo manually.
...
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "70%"}
newsite <- image_read("Making_Site_images/newsite.png")
newsite
```

You'll notice a few things happened!

* A .Rmd documented titled "hello-world.Rmd" automatically opened. 
* Your viewer window displays the homepage of your site.
* If you click the "Show in New Window" button on the top left of your viewer window, your website will open in your local web browser. *Note: this site is not yet "live", so you can only access it from your computer.* To return to R editing, click the Stop sign in the right hand side of the console window. 

That's what happened in R. Let's take a look at our working directory folder. 
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "50%"}
folders <- image_read("Making_Site_images/folders.png")
folders
```
...
Hugo takes all of the content from its archetypes, content, data, layouts, static, and themes folders (the ones that were just added to your local working directory) and uses them to build a website *inside* of the public folder inside that same directory.
...
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "30%"}
createbranch <- image_read("Making_Site_images/createbranch.png")
createbranch
```
...
6. Time to set up our sub branch. You have two options here. 
  + You can enter each of these code pieces step by step into your command line interface (terminal or git bash). Make sure to replace $USERNAME with your GitHub username and $SOURCE with sources throughout.
  ...
  *If you get an error about authentication, follow the GitHub authentication steps [here](https://help.github.com/categories/ssh/)*
...
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "70%"}
sourcesbranch <- image_read("Making_Site_images/sourcesbranch.png")
masterbranch <- image_read("Making_Site_images/masterbranch.png")
sourcesbranch
masterbranch
```  

If something seems to have gone wrong, you go back up to running the setup.sh script again. This will delete whatever you have in your master branch, so be careful!
...
My theme comes pre-setup with a Home page, an About page, and a Contact page. But say I'd like to add a Blog Page. 
...
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "30%"}
listfile <- image_read("Making_Site_images/listfile.png")
listfile
```
...
* **Easy option**: Find a new theme that does have a list template
* **Harder option**: Create your own list template that matches the theme

For my site (https://proquestionasker.github.io) I made a new list file to match this theme. If you're using the [Creative Portfolio Theme](http://themes.gohugo.io/hugo-creative-portfolio-theme/) and you'd like to use my list file, it's available [here](https://github.com/ProQuestionAsker/ProQuestionAsker.github.io/blob/sources/themes/hugo-creative-portfolio-theme/layouts/_default/list.html). To use it, copy the text into a text editor, save the file as list.html inside the layouts/_default folder. 
...
If you've been following along this far, you may have guessed by now that the place to add new files is inside the **content** folder in your local GitHub repo. **Do not manually add anything to the public folder**
...
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "30%"}
newblog <- image_read("Making_Site_images/newblog.png")
newblog
```
...
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "30%"}
newblog2 <- image_read("Making_Site_images/newblog2.png")
newblog2
```
...
The text in this view is the .Summary, or a Hugo-generated piece of text that includes that first 70 words of your file. To my knowledge, there's currently no way to adjust this inside .Rmd files. 
...
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "50%"}
newblog3 <- image_read("Making_Site_images/newblog3.png")
newblog3
```
...
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "30%"}
pfrontmatter <- image_read("Making_Site_images/pfrontmatter.png")
pfrontmatter
```

And our home page now contains this:
```{r,  echo = FALSE, message = FALSE, fig.align = "center", out.width = "30%"}
pfrontmatter2 <- image_read("Making_Site_images/pfrontmatter2.png")
pfrontmatter2
```
...
Check the baseurl in your config.toml file. It needs to be formatted like this: **baseurl = "https://proquestionasker.github.io/"** WITH the last / !!!
...
This one plagued me forever. Turns out that in the front matter of my document, I had draft: true.  **If Hugo thinks your post is a draft, it will not publish it.** 
